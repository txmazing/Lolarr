...
function Suspense(props: {
  ...
  return (
    <>
      ...
      <view hidden>{suspense() ? null : children}</view>
    </>
  );
}
...
function fadeIn(el: lng.ElementNode) {
  el.alpha = 0;
  el.animate({ alpha: 1 }, { duration: 250, easing: "ease-in-out" }).start();
}
    ...
    <>
      <view forwardFocus={0}>
        <Suspense
          fallback={
            <>
              <view onCreate={fadeIn} onDestroy={fadeOut} display="flex" center>
                <text>Loading...</text>
              </view>
            </>
          }
        >
          ...
          >
            <text>
              Hello World{"!".repeat(data() ?? 0)} (Press Enter to refetch)
            </text>
          </view>
        </Suspense>
      </view>
