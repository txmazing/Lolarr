...
describe('SeerrAdapter discover cache', () => {
  ...
  it('busts the discover cache after a successful deleteRequest so the next discover() hits upstream again', async () => {
    for (const path of ['/api/v1/discover/trending', '/api/v1/discover/movies', '/api/v1/discover/tv']) {
      ctx.seerr
        .intercept({ path, method: 'GET' })
        .reply(200, { results: [{ id: 1, mediaType: 'movie', title: 'Dune' }] }, { headers: { 'content-type': 'application/json' } })
    ...
    // deleteRequest() must invalidate the discover cache on success so poster badges
    // don't show stale "Requested" status for up to 5 minutes after canceling.
    ctx.seerr
      .intercept({ path: '/api/v1/request/42', method: 'DELETE' })
      .reply(204)

    await seerr.deleteRequest(userId, '42')
